	...
	testCase := func(r string, cases []testparams) {
		...
		for _, c := range cases {
			...
			if match && len(c.params) > 0 {
				assert.Equal(t, c.params[0:len(c.params)], ctxParams[0:len(c.params)], fmt.Sprintf("route: '%s', url: '%s'", r, c.url))
			}
		}
	}
...
func Test_RoutePatternMatch(t *testing.T) {
	...
	testCase := func(pattern string, cases []testparams) {
		for _, c := range cases {
			...
			assert.Equal(t, c.match, match, fmt.Sprintf("route: '%s', url: '%s'", pattern, c.url))
		}
	}
	testCase("/api/v1/:param/*", []testparams{
...
func Test_Utils_GetTrimmedParam(t *testing.T) {
	...
	assert.Equal(t, "param", res)
	res = GetTrimmedParam(":param1?")
	assert.Equal(t, "param1", res)
	res = GetTrimmedParam("noParam")
	assert.Equal(t, "noParam", res)
}
...
func Test_Utils_RemoveEscapeChar(t *testing.T) {
	...
	assert.Equal(t, "noEscapeChar", res)
}
	...
	benchCase := func(r string, cases []testparams) {
		...
		for _, c := range cases {
			...
			t.Run(r+" | "+state+" | "+c.url, func(b *testing.B) {
				...
				assert.Equal(t, c.match, matchRes, fmt.Sprintf("route: '%s', url: '%s'", r, c.url))
				if matchRes && len(c.params) > 0 {
					assert.Equal(t, c.params[0:len(c.params)-1], ctxParams[0:len(c.params)-1], fmt.Sprintf("route: '%s', url: '%s'", r, c.url))
				}
			})
		}
	}
	...
	benchCase := func(pattern string, cases []testparams) {
		for _, c := range cases {
			...
			t.Run(pattern+" | "+state+" | "+c.url, func(b *testing.B) {
				...
				assert.Equal(t, c.match, matchRes, fmt.Sprintf("route: '%s', url: '%s'", pattern, c.url))
			})
		}
	}
	benchCase("/api/:param/fixedEnd", []testparams{
